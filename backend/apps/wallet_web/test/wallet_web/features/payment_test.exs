defmodule WalletWeb.Features.PaymentTest do
  ...
  describe "successful transaction" do
    test "updates wallet balance", %{conn: conn} do
      ...
      assert_value [before: wallet_before["balance"], after: wallet_after["balance"]] == [before: %{"msatoshi" => 1000000000}, after: %{"msatoshi" => 950000000}]
    end
  end
  ...
  describe "non-sufficient funds" do
    test "results in declined transaction", %{conn: conn} do
      ...
      assert_value wallet_before["balance"] == %{"msatoshi" => 1000000000}

      # try to pay invoice -> declined
      invoice = "lntb11m...200" # 11 mBTC
      conn = post with_auth(conn, token1), transaction_path(conn, :create),
        invoice: invoice
      ...
      assert_value [before: wallet_before["balance"], after: wallet_after["balance"]] == [before: %{"msatoshi" => 1000000000}, after: %{"msatoshi" => 1000000000}]
    end
  end
end
